...
public class CustomUI : MonoBehaviour
{
    ...
    void UIUpdate()
    {
        ...
        Gray(); // 플레이어 버튼의 색상 업데이트
                // 현재 선택된 플레이어의 구매 여부에 따라 버튼 색상 업데이트
    }
    ...
    void buy(int value, bool check, string boolKey)
    {
    ...
    void CheckBought(int value, bool check, int Key, string boolKey)
    {
        ...
        else
        {
            ...
            buy(value, check, boolKey); // 아이템 구매 함수 호출
            UIUpdate(); // UI 업데이트 함수 호출
        }
    }
    ...
    void Gray()
    {
        if (!c1)
        {
            player1.GetComponent<Image>().color = LockedColor; // 플레이어 1 버튼 색상 변경
        }
        else
        {
            player1.GetComponent<Image>().color = UnlockedColor; // 플레이어 1 버튼 색상 변경
        }


        if (!c2)
        {
            player2.GetComponent<Image>().color = LockedColor; // 플레이어 1 버튼 색상 변경
        }
        else
        {
            player2.GetComponent<Image>().color = UnlockedColor; // 플레이어 1 버튼 색상 변경
        }


        if (!c3)
        {
            player3.GetComponent<Image>().color = LockedColor; // 플레이어 1 버튼 색상 변경
        }
        else
        {
            player3.GetComponent<Image>().color = UnlockedColor; // 플레이어 1 버튼 색상 변경
        }


        if (!c4)
        {
            player4.GetComponent<Image>().color = LockedColor; // 플레이어 1 버튼 색상 변경
        }
        else
        {
            player4.GetComponent<Image>().color = UnlockedColor; // 플레이어 1 버튼 색상 변경
        }


        if (!c5)
        {
            player5.GetComponent<Image>().color = LockedColor; // 플레이어 1 버튼 색상 변경
        }
        else
        {
            player5.GetComponent<Image>().color = UnlockedColor; // 플레이어 1 버튼 색상 변경
        }


        if (!c6)
        {
            player6.GetComponent<Image>().color = LockedColor; // 플레이어 1 버튼 색상 변경
        }
        else
        {
            player6.GetComponent<Image>().color = UnlockedColor; // 플레이어 1 버튼 색상 변경
        }
    }
}